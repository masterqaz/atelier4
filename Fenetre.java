...
public class Fenetre extends JFrame {
	...
	public class BoutonPresc3Listener implements ActionListener {
		public void actionPerformed(ActionEvent arg0) {
			String NAM = JOptionPane
					.showInputDialog(
							null,
							"Entre le numéro d'assurance maladie du client qui a besoin d'une prescription",
							"Ajout Prescription", JOptionPane.QUESTION_MESSAGE);
			String medicament = JOptionPane.showInputDialog(null,
					"Entre le nom du médicament", "Prescription",
					JOptionPane.QUESTION_MESSAGE);
			if (AjoutPrecriptionClient(NAM, medicament))

				JOptionPane.showMessageDialog(null, "Prescription ajouté!",
						"Prescription", JOptionPane.INFORMATION_MESSAGE);
			else
				JOptionPane.showMessageDialog(null,
						"Il n'est pas possible d'ajouté la prescription",
						"Prescription", JOptionPane.INFORMATION_MESSAGE);
		}
	}
	...
	public boolean AjoutPrecriptionClient(String NAM, String Medicament) {
		for (Iterator<Client> it = pharma.getLesClients().iterator(); it
				.hasNext();) {
			Client itClient = it.next();
			if (itClient.getNAM().equals(NAM)) {
				for (Iterator<Medicament> its = pharma.getLesMedicaments()
						.iterator(); its.hasNext();) {
					Medicament itMedicament = its.next();
					if (itMedicament.getNomMarque().equals(Medicament)) {
						String dose = JOptionPane.showInputDialog(null,
								"Entre la dose de la prescription",
								"Dose Prescription",
								JOptionPane.QUESTION_MESSAGE);
						String renouvellementsRestants = JOptionPane
								.showInputDialog(
										null,
										"Entre le nombre de renouvelement restant",
										"Prescription",
										JOptionPane.QUESTION_MESSAGE);
						Prescription prescription = new Prescription(
								Medicament, Double.parseDouble(dose),
								Integer.parseInt(renouvellementsRestants));
						for (int i = 0; i < itClient.getPrescriptions().size(); i++) {
							if (itClient.getPrescriptions().get(i)
									.getMedicamentAPrendre()
									.equals(itMedicament.getNomMarque())) {
								itClient.getPrescriptions().remove(i);
	...
	public String TrouveDoseSuggerer(String Medicament, String NAM) {
		String result = "les doses sugéré sont : ";
		double dose = -1;
		for (Iterator<Client> it = pharma.getLesClients().iterator(); it
				.hasNext();) {
			Client itClient = it.next();
					...
					if (itMedicament.getNomMarque().equals(Medicament)) {
						for (int i = 0; i < itClient.getPrescriptions().size(); i++) {
							if (itClient.getPrescriptions().get(i)
									.getMedicamentAPrendre()
									.equals(itMedicament.getNomMarque())) {
								dose = itClient.getPrescriptions().get(i)
										.getDose();
								break;
							}
						}
		...
		if (dose != -1) {
			for (Iterator<Medicament> its = pharma.getLesMedicaments()
				...
				Medicament itMedicament = its.next();
				if (itMedicament.getNomMarque().equals(Medicament)) {
					double dosesPossibles[] = itMedicament.getDosesPossibles();
					for (int j = dosesPossibles.length - 1; j >= 0
							&& dose >= dosesPossibles[j]; j--) {
						System.out.println(dose);
						if (dosesPossibles[j] != 0) {
							dose -= dosesPossibles[j];
							result += dosesPossibles[j] + ", ";
						}
						if (dose == 0){
							break;
						}
